...
/**
 * Gestisce i danni ricevuti dalle entit├á (NPC o giocatori)
 */
export class EntityDamagedHandler extends BaseMessageHandler {
  ...
  handle(message: EntityDamagedMessage, networkSystem: ClientNetworkSystem): void {
    ...
    // ­ƒöº NUOVA LOGICA: Aggiorna il cooldown del laser quando il player locale infligge danno
    ...
    if (isLocalPlayerAttacker && message.projectileType === 'laser') {
      // Trova l'entit├á del player locale e aggiorna lastAttackTime
    ...
    if (combatSystem) {
      // Trova l'entit├á danneggiata
      let targetEntity = null;

      if (message.entityType === 'npc') {
        // Usa il RemoteNpcSystem per trovare l'entit├á dell'NPC remoto
        ...
        if (remoteNpcSystem) {
          ...
          if (entityId !== undefined) {
            // Ottieni l'entit├á effettiva dall'ECS usando l'entity ID
        ...
        if (message.entityId === networkSystem.getLocalClientId()) {
          // Giocatore locale - trova l'entit├á locale
          ...
          // Giocatore remoto - trova l'entit├á remota
          const allEntities = ecs.getEntitiesWithComponents(Health);
          for (const entity of allEntities) {
            const remotePlayer = ecs.getComponent(entity, RemotePlayer);
            // ­ƒÜÇ FIX ROBUSTEZZA: Forza il confronto tra stringhe per evitare problemi tra number e string (clientId)
      ...
      if (message.damage > 0) {
        ...
        // Passa anche la posizione dal server per garantire che il testo appaia anche se l'entit├á ├¿ stata distrutta (es: oneshot)
    ...
    // Nota: l'aggiornamento dei valori health/shield ├¿ gi├á stato fatto sopra
    ...
    if (message.entityType === 'player') {
      ...
      if (message.entityId === networkSystem.getLocalClientId()) {
        ...
        if (ecs) {
          ...
          // Cerchiamo entit├á che hanno Health e Shield ma non RemotePlayer
          const allEntities = ecs.getEntitiesWithComponents(Health, Shield);
          ...
          for (const entity of allEntities) {
            ...
            // Questa dovrebbe essere l'entit├á del giocatore locale
