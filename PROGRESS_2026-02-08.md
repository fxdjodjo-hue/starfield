...
- `server/core/InputValidator.cjs`
  - `validateCombat` now validates `clientId` instead of `playerId`.
- `server/core/connection/MessageRouter.cjs`
  - `projectile_fired` broadcast uses server authoritative `userId`.
  - `start_combat` and `stop_combat` broadcasts use server authoritative `userId`.
  - Debug log for start_combat now prints server-side userId.
- `src/multiplayer/client/managers/NetworkCombatManager.ts`
  - `start_combat` / `stop_combat` / `projectile_fired` no longer send `playerId`.
- `src/config/NetworkConfig.ts`
  - Start/Stop/Projectile message types updated for clientId-first; `playerId` treated as legacy.
- `src/systems/ui/managers/UINicknameManager.ts`
  - User-owned changes (do not touch).

**Remaining Next Steps (when ready)**
- Review and commit the combat payload alignment changes (exclude the UI file).
- Smoke test: start combat, fire projectile, stop combat; verify no client errors and server logs clean.
- Optional: clean up `StopCombatMessage` typing if we want to distinguish client->server vs server->client payloads.

**How To Resume Tomorrow**
1) `git status -sb` and confirm only the intended files are staged/unstaged.
2) Finish your UI file changes.
3) Stage only:
   - `server/core/InputValidator.cjs`
   - `server/core/connection/MessageRouter.cjs`
   - `src/multiplayer/client/managers/NetworkCombatManager.ts`
   - `src/config/NetworkConfig.ts`
4) Commit with a message like: `Align combat payloads to clientId-only`.
5) Quick test in game: start combat, shoot, stop combat.
