...
serve(async (req) => {
    ...
    try {
        ...
        if (stats) {
            updates.push(supabaseClient.from('player_stats').upsert({
                user_id: user.id,
                kills: stats.kills || 0,
        ...
        if (upgrades) {
            updates.push(supabaseClient.from('player_upgrades').upsert({
                user_id: user.id,
        ...
        if (currencies) {
            updates.push(supabaseClient.from('player_currencies').upsert({
                user_id: user.id,
        ...
        if (quests && Array.isArray(quests)) {
            const questUpdates = quests.map((quest) => supabaseClient.from('quest_progress').upsert({
                user_id: user.id,
