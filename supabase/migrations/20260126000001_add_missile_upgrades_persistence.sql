-- Migration: Add Missile Upgrades Persistence (FINAL STABLE VERSION)

-- 1. Ensure column exists in public.player_upgrades
ALTER TABLE public.player_upgrades ADD COLUMN IF NOT EXISTS missile_damage_upgrades INTEGER DEFAULT 0;

-- 2. Update get_player_data_secure (Used by various server systems)
...
-- 3. Update update_player_data_secure (COMPLETE LOGIC - NO PLACEHOLDERS)
...
BEGIN
  ...
  IF stats_data IS NOT NULL THEN
    INSERT INTO public.player_stats (
      ...
      auth_id_param,
      COALESCE((stats_data->>'kills')::INTEGER, 0),
      COALESCE((stats_data->>'deaths')::INTEGER, 0),
      COALESCE((stats_data->>'missions_completed')::INTEGER, 0),
      COALESCE((stats_data->>'play_time')::INTEGER, 0)
    )
  ...
  -- Update upgrades (Supports BOTH camelCase from JS and snake_case for safety)
  IF upgrades_data IS NOT NULL THEN
    INSERT INTO public.player_upgrades (
      ...
      auth_id_param,
      COALESCE((upgrades_data->>'hpUpgrades')::INTEGER, (upgrades_data->>'hp_upgrades')::INTEGER, 0),
      COALESCE((upgrades_data->>'shieldUpgrades')::INTEGER, (upgrades_data->>'shield_upgrades')::INTEGER, 0),
      COALESCE((upgrades_data->>'speedUpgrades')::INTEGER, (upgrades_data->>'speed_upgrades')::INTEGER, 0),
      COALESCE((upgrades_data->>'damageUpgrades')::INTEGER, (upgrades_data->>'damage_upgrades')::INTEGER, 0),
      COALESCE((upgrades_data->>'missileDamageUpgrades')::INTEGER, (upgrades_data->>'missile_damage_upgrades')::INTEGER, 0)
    )
    ON CONFLICT (auth_id) DO UPDATE SET
  ...
  -- Update currencies (Including current health/shield for true MMO persistence)
  IF currencies_data IS NOT NULL THEN
    INSERT INTO public.player_currencies (
      ...
      COALESCE((currencies_data->>'current_health')::INTEGER, 10000),
      COALESCE((currencies_data->>'current_shield')::INTEGER, 5000)
    )
    ON CONFLICT (auth_id) DO UPDATE SET
...
-- 4. Update get_player_complete_data_secure (Used during login/welcome)
...
BEGIN
  -- 1. Get total population for percentile calculation
  SELECT COUNT(*) INTO v_total_players 
  FROM public.user_profiles up 
  WHERE COALESCE(up.is_administrator, FALSE) = FALSE;

  -- 2. Get player position based on honor score
  ...
  SELECT
    ...
    COALESCE(
      ...
      )::text FROM public.quest_progress qp WHERE qp.auth_id = up.auth_id),
      '[]'
    ) as quests_data
  INTO result_record
  FROM public.user_profiles up
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT 
      ...
      NULL::TEXT, NULL::TEXT, '[]', 'Basic Space Pilot'::VARCHAR;
  ELSE
    RETURN QUERY SELECT 
